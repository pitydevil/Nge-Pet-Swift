...
extension BaseProviders  {
    ...
    //MARK: - UPDATE EXISTING PET DATA BASED ON PETID ARGUMENTS
    func updateExisting(_ petID : UUID, _ petAge : Int16, _ petDataIndex: Int16, _ petBreed : String, _ petGender : String,_ petName : String ,_ petSize : String,_ petType : String, completion: @escaping(_ result: Bool) -> Void) {
        let taskContext = newTaskContext()
    ...
    func addPet(_ petID : UUID, _ petAge : Int16, _ petDataIndex: Int16, _ petBreed : String, _ petGender : String,_ petName : String ,_ petSize : String,_ petType : String, _ dateCreated : Date ,completion: @escaping(_ result: Bool) -> Void) {
        let taskContext = newTaskContext()
        taskContext.perform {
            do {
                if let entity = NSEntityDescription.entity(forEntityName: "Pet", in: taskContext) {
                    ...
                    member.setValue(petDataIndex, forKeyPath: "petDataIndex")
        ...
        return Observable<T>.create { observer in
            ...
            taskContext.perform {
                ...
                do {
                    ...
                    for pet in results {
                        let pet = Pets(petID: pet.value(forKey: "petID") as? UUID, petDataIndex: pet.value(forKey: "petDataIndex") as? Int16, petAge: pet.value(forKey: "petAge") as? Int16, petBreed: pet.value(forKey: "petBreed") as? String, petName: pet.value(forKey: "petName") as? String, petSize: pet.value(forKey: "petSize") as? String, petType: pet.value(forKey: "petType") as? String, petGender: pet.value(forKey: "petGender") as? String, dateCreated : pet.value(forKey: "dateCreated") as? Date)
                        petArray.append(pet)
                    }
